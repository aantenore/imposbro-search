...
- **Tenant policy**: Use `AUTHZ_COLLECTION_POLICIES` for collection-level tenant enforcement. Tenant search filters, ingest validation/injection, and delete filters must happen server-side in `auth.py`, not in the Admin UI or client code.
