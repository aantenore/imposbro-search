...
32. **Kubernetes benchmark harness**: `make benchmark-k8s` runs configurable sustained ingest/search against a deployed Query API, waits for indexing visibility, records p50/p95/p99 latencies, emits JSON/Markdown artifacts with run metadata, and can enforce environment-specific SLO thresholds.
...
40. **Routing policy v2**: Document routing now supports exact, in-list, glob, and numeric range operators with priority ordering, fan-out targets, and dry-run preview from both Admin API and Admin UI.
...
- **Operational scale proof**: local Docker now covers multi-instance rolling smoke, lag budget, JSON/Markdown benchmark artifacts with run metadata, Helm autoscaling manifests, and a repeatable Kubernetes benchmark harness; the next credibility step is publishing results from a production-sized Kubernetes run.
